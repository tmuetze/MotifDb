...
{
  dataDir <- file.path(dataDir) # Comment:Tanja: I took out "HOCOMOCO" after dataDir
...
createMetadataTable = function (dataDir, matrices, raw.metadata.filename)
{
  ...
  for (matrix.id in matrix.ids) {
    ...
    shorter.matrix.name <- unlist(strsplit(short.matrix.name, "_"))[1]
    
    if (grepl(shorter.matrix.name, "+")){
      split.matrix.name <- unlist(strsplit(shorter.matrix.name, "+"))[-1]
      shorter.matrix.name <- split.matrix.name
    }
    

    protID <- paste0(shorter.matrix.name, "_HUMAN")
